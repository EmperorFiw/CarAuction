...
{
	new targetid;
	if(sscanf(params,"d",targetid)) return SendClientMessage(playerid, COLOR_GREY, "{FF6142}USAGE: /acceptshake [ไอดีผู้เล่น/ชื่อบางส่วน]");
...
CMD:no(playerid, params[])
{
    if (AnimationCheck(playerid))
        return SendClientMessage(playerid, COLOR_GREY, "ไม่สามารถเล่น Animation ได้ในขณะนี้");
...
CMD:bj(playerid, params[])
{
	...
	if(sscanf(params, "s[32]", choice))
	{
		SendClientMessage(playerid, COLOR_GREY, "การใช้: /bj [1-4]");
		return 1;
	}
	if(strcmp(choice, "1", true) == 0)
	{
